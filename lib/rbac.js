...
class RBAC {
  ...
  _findRolesMatchingOperation(operation, role) {
    if (!isGlob(operation)) {
      return []
    }

    const r = globToRegex(operation)
    return (role.canGlob || []).filter(p => {
      const permissionTokens = p.original.split(':')
      const operationTokens = operation.split(':')
      const interpolatedPermission = permissionTokens.map((token, index) => {
        if (token === '*' && operationTokens[index]) {
          return operationTokens[index]
        }
        return token
      }).join(':')

      return r.test(interpolatedPermission)
     }).map(p => p.original)
  }
  ...
  async can(role, operation, params, cb) {
    ...
    if (!$role.can[operation] && !$role.canGlob.find(glob => glob.name.test(operation))) {
      ...
      if (!$role.inherits || $role.inherits.length < 1) {
        ...
        const permissionsMatches = this._findRolesMatchingOperation(operation, $role)
        return permissionsMatches.length ? permissionsMatches: false
      }
    ...
    if ($role.can[operation] === 1) {
      ...
      const permissionsMatches = this._findRolesMatchingOperation(operation, $role)
      return permissionsMatches.length ? permissionsMatches : true
    }
    ...
    if (globMatch && globMatch.when) {
      ...
      try {
        ...
        debug('conditional function threw', e)
        return this._findRolesMatchingOperation(operation, $role)
      }
    }
